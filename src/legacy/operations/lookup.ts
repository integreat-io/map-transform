...
type GetPropFn = (val: unknown, state: StateClass) => unknown
...
const createLookupFn = (
  next: StateMapper,
  matchFn: (state: StateClass) => (value: unknown) => Promise<unknown>,
  extractProp: (state: State) => (value: unknown) => Promise<unknown>,
  doFlip: boolean,
) =>
  async function doLookup(state: State) {
    const nextState = new StateClass(await next(state))
...
export function lookup({
  ...
  return (options) => {
    ...
    const extractProp = (state: State) => async (value: unknown) =>
      getter(value, new StateClass(state))
