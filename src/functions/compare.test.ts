...
test('should return true when object has match value at path', (t) => {
  ...
  const ret = compare({ path, operator: '=', match })(data, context)
...
test('should use matchPath to get match value from data', (t) => {
  ...
  const ret = compare({ path, operator: '=', matchPath })(data, context)
...
test('should use equality as default operator', (t) => {
  ...
  const ret = compare({ path, match })(data, context)
...
test('should use . as path when no path', (t) => {
  ...
  const ret = compare({ match })(data, context)
...
test('should support higher and lower than', (t) => {
  ...
  t.true(compare({ path: 'age', operator: '>', match: 18 })(data, context))
  t.true(compare({ path: 'age', operator: '<', match: 40 })(data, context))
  t.true(compare({ path: 'age', operator: '>=', match: 36 })(data, context))
  t.true(compare({ path: 'age', operator: '<=', match: 36 })(data, context))
  t.false(compare({ path: 'age', operator: '>', match: 36 })(data, context))
  t.false(compare({ path: 'age', operator: '<', match: 36 })(data, context))
...
test('should support higher and lower than in array', (t) => {
  ...
  t.true(compare({ path: 'age', operator: '>=', match: 36 })(data, context))
  t.true(compare({ path: 'age', operator: '<=', match: 36 })(data, context))
  t.true(compare({ path: 'age', operator: '<', match: 12 })(data, context))
  t.false(compare({ path: 'age', operator: '>', match: 36 })(data, context))
  t.false(compare({ path: 'age', operator: '<', match: 10 })(data, context))
  t.false(compare({ path: 'age', operator: '>=', match: 37 })(data, context))
  t.false(compare({ path: 'age', operator: '<=', match: 9 })(data, context))
})

test('should support require numeric value and match', (t) => {
  const data = { name: 'John F.', age: 36 }

  t.false(compare({ path: 'name', operator: '>', match: 36 })(data, context))
  t.false(compare({ path: 'name', operator: '<', match: 36 })(data, context))
  t.false(compare({ path: 'age', operator: '>=', match: '37' })(data, context))
  t.false(compare({ path: 'age', operator: '<=', match: '35' })(data, context))
})
