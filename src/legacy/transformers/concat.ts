...
function* concatGen(
  ...
  if (revFromState(state)) {
    ...
    for (const getFn of getFns.slice(1)) {
      merged = yield getFn([], new StateClass({ ...state, target: merged }))
    }
    return merged
  } else {
    ...
    return merged.filter((val) => !isNonvalue(val, state.nonvalues))
  }
}
...
export const concat: Transformer = function concat({ path = [] }: Props) {
  return (options) => {
    ...
    return function concat(value, state) {
      const it = concatGen(value, new StateClass(state), getFns)
