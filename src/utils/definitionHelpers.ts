...
import { Operation, StateMapper, Operands, Data, DataMapper, MapDefinition, MapObject, Path, MapPipe, OperationObject, TransformObject, FilterObject, Options, CustomFunction, CustomFunctions } from '../types'
...
export const hasTransformProp = (def: MapObject | OperationObject): def is TransformObject => typeof def['$transform'] !== 'undefined'
export const hasFilterProp = (def: MapObject | OperationObject): def is FilterObject => typeof def['$filter'] !== 'undefined'
export const hasOperationProps = (def: MapObject | OperationObject): def is OperationObject => hasTransformProp(def) || hasFilterProp(def)

export const isPath = (def: any): def is Path => typeof def === 'string'
export const isMapObject = (def: any): def is MapObject => isObject(def) && !hasOperationProps(def)
...
const getOperationFunction = (fnId?: string, customFunctions?: CustomFunctions) =>
  (fnId && customFunctions && typeof customFunctions[fnId] === 'function')
    ? customFunctions[fnId] as CustomFunction<Operands, Data, Data | boolean> : undefined
...
const createOperation = <T>(
  ...
  return (typeof fn !== 'undefined')
    ? operationFn(fn(operands) as any)(options) : identity // TODO: Improve typing
}

const operationFromObject = (def: OperationObject | MapObject, options: Options) => {
...
export const mapFunctionFromDef = (def: MapDefinition, options: Options): StateMapper =>
  isMapPipe(def) ? pipe(def)(options)
    : isObject(def) ? operationFromObject(def, options)
